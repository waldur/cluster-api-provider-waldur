...
// RancherVaultParams holds the per-node Vault AppRole credentials for rancher-system-agent delivery.
type RancherVaultParams struct {
	Addr       string
	SecretPath string // KV v2 path where Crossplane stored the Rancher registration token
	RoleID     string
	SecretID   string // per-VM, generated by controller
}

// RancherAgentInput is the input to BuildRancherAgentUserData.
type RancherAgentInput struct {
	// StaticCloudInit is the base OS cloud-init template. May be nil/empty.
	StaticCloudInit []byte
	// VaultParams contains the per-node AppRole credentials and Vault token path.
	VaultParams RancherVaultParams
	// RancherURL is the Rancher server URL embedded directly in cloud-init.
	// The VM uses this to write its rancher-agent config before starting the agent.
	RancherURL string
}

// BuildRancherAgentUserData produces the final user_data for a VM that uses
// rancher-system-agent to join the cluster. The agent connects outbound to Rancher
// on port 443 and receives its node plan (RKE2 role, join token, server URL).
//
// The cloud-init:
//  1. Writes AppRole credentials to /etc/vault/{role-id,secret-id}.
//  2. Runs vault-fetch-rancher-registration.sh which authenticates to Vault,
//     fetches the Rancher registration token, writes /etc/rancher-agent/config.yaml,
//     and deletes the Vault credentials from disk.
//  3. Installs and starts rancher-system-agent via the registration URL.
func BuildRancherAgentUserData(in RancherAgentInput) (string, error) {
	var static CloudInit
...
# Fetch Rancher registration token from Vault using AppRole credentials.
# Runs as runcmd[0] — before rancher-system-agent starts.
set -e
...
RANCHER_TOKEN=$(curl -sf "${VAULT_ADDR}/v1/${VAULT_SECRET_PATH}" \
  -H "X-Vault-Token: ${VAULT_TOKEN}" \
  | grep -o '"clusterToken":"[^"]*"' | sed 's/"clusterToken":"//;s/"//')
...
`, in.VaultParams.Addr, in.VaultParams.SecretPath, in.RancherURL)

	rancherFiles := []CloudInitFile{
		{
		...
		{
			Path:        "/var/opt/scripts/vault-fetch-rancher-registration.sh",
			Permissions: "0755",
			Content:     fetchScript,
		},
	}
	...
	merged := CloudInit{
		WriteFiles: append(rancherFiles, static.WriteFiles...),
		RunCmd: append([]any{
			"/var/opt/scripts/vault-fetch-rancher-registration.sh",
			"systemctl enable --now rancher-system-agent",
		}, static.RunCmd...),
		BootCmd: static.BootCmd,
	}
